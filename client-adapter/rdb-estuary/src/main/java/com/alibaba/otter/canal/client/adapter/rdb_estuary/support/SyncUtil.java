package com.alibaba.otter.canal.client.adapter.rdb_estuary.support;
...
public class SyncUtil {
    ...
    public static String getDbTableName(MappingConfig.DbMapping dbMapping) {
        return getDbTableName(dbMapping.getTargetDb(), dbMapping.getTargetTable());
    }

    public static String getDbTableName(String dbName, String tableName){
        String result = "";
    ...
    /**
     *
     * @param sourceSql 参数不能包含where
     * @param wherePart
     * @return
     */
    public static String injectionWhere(String sourceSql, String wherePart){
        MySqlSelectQueryBlock queryBlock = SqlParser.parseSQLSelectQueryBlock(sourceSql);
     ...
     * @return
     */
    public static Map<String, Integer> getColumnTypeMap(DataSource dataSource, String table){
        Map<String, Integer> columnType = new LinkedHashMap<>();
        Util.sqlRS(dataSource, "SELECT * FROM " + table + " WHERE 1=2 ", rs -> {
            try {
                ...
                for (int i = 1; i <= columnCount; i++) {
                    //FIXME:列名可能存在大小写问题
                    columnType.put(rsd.getColumnName(i), rsd.getColumnType(i));
                }

...
import com.alibaba.otter.canal.client.adapter.rdb_estuary.config.SchemaItem.ColumnItem;
import com.alibaba.otter.canal.client.adapter.rdb_estuary.config.SchemaItem.FieldItem;
import com.alibaba.otter.canal.client.adapter.rdb_estuary.config.SchemaItem.RelationFieldsPair;
import com.alibaba.otter.canal.client.adapter.rdb_estuary.config.SchemaItem.TableItem;
...
public class SqlParser {
    ...
    public static void main(String[] args) {
        String sql = "select t1.id, t1.region, count(t2.depart_no) co\n" +
                "from t_shop_gxb t1\n" +
                "left join (\n" +
                "\tselect id id2, depart_no\n" +
                "\tfrom t_department_gxb\n" +
                "\twhere delflag = 0\n" +
                ") t2\n" +
                "on t1.region = t2.id2 and t1.region = t2.id2\n" +
                "group by t1.id, t1.region";
        SchemaItem item = parse(sql);
        return;
    }
    ...
    public static SchemaItem parse(String sql) {
        try {
            ...
            List<FieldItem> groupItems = collectGroupFields(sqlSelectQueryBlock);
            schemaItem.setGroupByFields(groupItems);
     ...
     * @return
     */
    private static List<FieldItem> collectGroupFields(MySqlSelectQueryBlock sqlSelectQueryBlock) {
        SQLSelectGroupByClause groupBy = sqlSelectQueryBlock.getGroupBy();
        if (groupBy == null){
            return new ArrayList<>();
        }
        return groupBy.getItems().stream().map(groupItem -> {
            FieldItem fieldItem = new FieldItem();
            fieldItem.setExpr(groupItem.toString());
            visitColumn(groupItem, fieldItem);
            return fieldItem;
        }).collect(Collectors.toList());
    }
